...
import { postAsUser, putAsUser, useUserOptions } from "../utils/data";
  ...
  return (
    <Formik
      ...
      validationSchema={Yup.object().shape({
        ...
        category: Yup.string().min(1).max(100).required("Category is required"),
      ...
      onSubmit={async (values, { setSubmitting }) => {
        try {
          ...
          let message, ok;
          if (!transaction._id) {
            const data = await postAsUser("/api/v1/transactions", payload);
            message = data.message;
            ok = data.ok;
          } else {
            const data = await putAsUser(
        ...
        <Form className="d-flex flex-column">
          ...
          <FormGroup>
            ...
            {errors.description && touched.description ? (
              <FormText className="text-danger">{errors.description}</FormText>
            ) : null}
          </FormGroup>
          <FormGroup>
              ...
              {({ field }) => (
                <BootstrapForm.Check type="checkbox" {...field}>
                  <BootstrapForm.Check.Input
                    ...
                    onChange={(event) =>
                      setFieldValue("includePayerInSplit", event.target.checked)
                    }
                    onBlur={() => setFieldTouched("includePayerInSplit", true)}
                  />
